...
class CreateGameView(APIView):
    ...
    def post(self, request):
        ...
        ----------
        request : Request
            Data object. Fields that represent a User's attempt to create a new game. 
        
        Returns
        -------
        JsonResponse
            Successful creation of a new game.

        Raises
        ------
        JsonResponse
            Failed creation of a new game due to missing or invalid data. Either due to failing to specify a game duration, or 
            due to needing a starting balance of at least 1000 dollars. 
        """
        data = request.data
...
class JoinGameView(APIView):
    ...
    def post(self, request):
        ...
        ----------
        request : Request
            Data object. Fields that represent a User's attempt to join a game. 
        
        Returns
        -------
        JsonResponse
            Successful join game response with game_id or error message.

        Raises
        ------
        JsonResponse, status= 404
            Failed join game due to an exception. Either game is over or user already joined.
...
class GetGameAndPlayerData(APIView):
    ...
    def post(self, request):
        """
        Post request to get game and player data.
        
        Parameters
        ----------
        request : Request
            Data object. Fields that represent a User's request to get game and player data. 
        
        Returns
        -------
        JsonResponse
            Successful retrieval of game and player data.

        Raises
        ------
        JsonResponse
            Failed retrieval of game and player data due to unexpected errors. 
        """
        user = request.user
...
class UserGamesView(APIView):
    ...
    def get(self, request):
        """
        Handles GET request to retrieve all games associated with a user.
        
        Parameters
        ----------
        request : Request
            The request object that contains all the HTTP information.
        
        Returns
        -------
        JsonResponse
            A JSON response containing all games associated with the user.
...
class InteractWithHolding(APIView):
    ...
    def post(self, request):
        """
        Handles POST request to buy or sell stocks for a user. Updates player balance and portfolio in the database as well. 
        ...
        -------
        JsonResponse
            A JSON response containing a success message or an error message.

        Raises
        ------
        JsonResponse, status=400
            Invalid input data or insufficient balance.
        
        JsonResponse, status=500
            Unable to fetch live price for the ticker.
        """
        data = request.data
...
class GetScoreboard(APIView):
    ...
    def post(self, request):
        """
        Post request to get the scoreboard of a game.
        
        Parameters
        ----------
        request : Request
            Data object. Fields that represent a User's request to get the scoreboard of a game. 
        
        Returns
        -------
        JsonResponse
            Successful retrieval of the scoreboard.
        """
        game_id = request.data.get("game_id")
